...
export function VenueDetailsPageV2({
  ...
  const venue = {
    name: venueData.name,
    location:
      venueData.version?.data?.step1?.location?.formattedAddress ||
      venueData.destination ||
      "Unknown Location",
    rating: 4.8,
    reviews: 124,
    venueTags: venueData.version?.data?.step1?.interests || [],
    description:
      venueData.version?.data?.step1?.overview || "No description available.",
    whyCouplesLove: Object.entries(
      venueData.version?.data?.step2?.dynamicServicesData?.[
        "Venue Highlights"
      ] || {},
    )
    ...
    localPrices: (
      venueData.version?.data?.step2?.dynamicServicesData?.["Rooms and Stay"] ||
      []
    ).map((room: any) => ({
      item: room.room_type,
    ...
    goodToKnow: Object.entries(
      venueData.version?.data?.step4?.rulesPolicies || {},
    ).map(([key, value]) => {
    ...
    images:
      venueData.version?.data?.step1?.coverPhotosWeb?.map(
        (p: any) => p.fileUrl,
      ) || [],
    capacity: {
      min:
        parseInt(
          venueData.version?.data?.step2?.dynamicServicesData?.[
            "Basic Details"
          ]?.["maximum_seating_capacity"],
        ) || 50,
      max:
        parseInt(
          venueData.version?.data?.step2?.dynamicServicesData?.[
            "Basic Details"
          ]?.["max_pax_allowed_maximum_head_count"],
        ) || 200,
    },
    pricing: {
      starting:
        venueData.version?.data?.step3?.packages?.[0]?.packagePrice?.amount ||
        0,
      currency:
        venueData.version?.data?.step3?.packages?.[0]?.packagePrice?.currency ||
        "USD",
    },
    ...
    packages: (venueData.version?.data?.step3?.packages || []).map(
      (pkg: any) => ({
        name: pkg.packageName,
        price: pkg.packagePrice,
        tag: pkg.packageName,
        image:
          pkg.packagePhotos?.[0]?.fileUrl ||
          venueData.version?.data?.step1?.coverPhotosWeb?.[0]?.fileUrl,
        images: pkg.packagePhotos?.map((p: any) => p.fileUrl) || [],
        numberOfDays: pkg.numberOfDays || 1,
        totalPax: pkg.totalPax || 0,
        numberOfRooms:
          pkg.roomTypes
            ?.map((r: any) => `${r.type} (${r.roomCount})`)
            .join(", ") || "N/A",
  ...
  // Debug log to check the mapped data structure
  const venueId = venueData._id;
  console.log(typeof venueId, venueId);
  ...
  const { addToCompare, removeFromCompare, isInCompare, comparePackages } =
    usePackageCompare();
      ...
      <div className="container mx-auto px-4 md:px-8 py-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8 min-w-0 overflow-hidden">
                        ...
                        {areasExpanded ? (
                          ...
                          <>
                            <span className="hidden sm:inline">
                              Show More ({venue.areas.length - 4} more)
                            </span>
                            <span className="sm:hidden">
                              +{venue.areas.length - 4} More
                            </span>
                            <ChevronDown className="size-4" />
                          </>
                        )}
                  ...
                  >
                    <div className="grid md:grid-cols-[280px,1fr] gap-3 sm:gap-4 md:gap-6 p-3 sm:p-4 md:p-6 w-full min-w-0">
                      ...
                      <div
                        ...
                        onClick={() => {
                          const packageImages = (pkg as any).images || [
                            pkg.image,
                          ];
                          openLightbox(
                        ...
                        <div className="absolute top-3 right-3 flex gap-2 z-10">
                          <button
                            ...
                            className={`p-2 rounded-full bg-white/90 hover:bg-white shadow-lg transition-all hover:scale-110 ${
                              likedPackages.has(`${venueId}-${pkg.name}`)
                                ? "text-red-500"
                                : ""
                            }`}
                          >
            ...
            {/* Map */}
            <div>
              <h2 className="mb-4 sm:mb-5 md:mb-6 text-lg sm:text-xl">
                Location
              </h2>
              <SimpleDestinationMap
                center={[
                  venue.coordinates.lat || 0,
                  venue.coordinates.lng || 0,
                ]}
              ...
              >
                ...
                <div className="flex flex-col items-start">
                  ...
                  <span className="text-[10px] md:text-xs opacity-90">
                    {comparePackages.length}{" "}
                    {comparePackages.length === 1 ? "package" : "packages"}{" "}
                    added
                  </span>
                </div>
              </Button>
